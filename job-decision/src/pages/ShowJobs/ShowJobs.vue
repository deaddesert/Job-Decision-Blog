...
import ChoiceBoxes from '../Tools/choices.vue';


export default{
	
	components:{
		'choice-box': ChoiceBoxes,

		},

	computed:{
		updatedGrade() {
			return this.$store.state.form.newGrade
		},

		updatedDomain() {
			return this.$store.state.form.newDomain
		},

		updatedGroup() {
			return this.$store.state.form.newGroup
		}
	},
	...
	apollo:{
		// Query
		allJobs:{
			query: gql `query allJobs($subjects: String!, $domain: String!, $field: String!) {
							allJobs(subjects_Icontains: $subjects, domain_Icontains: $domain, field_Icontains: $field){
								id
								name
								subjects
								domain
			...
			variables(){
				return{
					subjects : this.updatedGrade,
					domain : this.updatedDomain,
					field : this.updatedGroup
				}
			},

			skip() {
				return this.$store.state.skipQuery
			}
